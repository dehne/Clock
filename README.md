...
The main method for a Clock object is driveMicros(). This method advances the clock mechanism it 
represents by the number of microseconds it is passed as an argument. This method is intended to be
invoked repeatedly in the loop() of an Arduino sketch that has calculated the amount of time that has
passed since driveMicros() was last invoked. The driveMicros() method keeps track of these passing 
microseconds and advances the movement to keep the clock to within 1/2 second of correct. 
